...
def get_batchtools_path():
    ...
    if not os.environ.get(SAT_env) is None:
        ...
        print(
            "Overriding path to batch tools using path: %s from environment variable %s"
            % (path, SAT_env)
        )
        return path
...
def main(initial_path):
    ...
    samples_path = {
        _dir: os.path.join(base_samples_path, _dir) for _dir in sample_dir_names
    }
    output_path = {
